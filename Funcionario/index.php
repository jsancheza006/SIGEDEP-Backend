 ...
 *   HTTP 200
 *   [
 *     {"ID_Funcionario": 1, "Nombre": "Juan", "Apellido": "Pérez", "correo": "juan@example.com", "Numero": "123456"},
 *     {"ID_Funcionario": 2, "Nombre": "María", "Apellido": "López", "correo": "maria@example.com", "Numero": "789012"}
 *   ]
 * 
...
if ($method === 'GET') {
    ...
    if (isset($query['id'])) {
        ...
        $stmt = $conn->prepare("SELECT ID_Funcionario, Nombre, Apellido, correo, Numero FROM funcionario WHERE ID_Funcionario = ?");
    ...
    $result = $conn->query("SELECT ID_Funcionario, Nombre, Apellido, correo, Numero FROM funcionario");
...
if ($method === 'POST') {
    ...
    $correo = $body['correo'] ?? null;            // Obligatorio
    $numero = $body['Numero'] ?? null;            // Obligatorio

    // Valida que los campos obligatorios estén presentes
    if (!$nombre || !$apellido || !$correo || !$numero) {
        http_response_code(400);
        echo json_encode(['error' => 'Los campos Nombre, Apellido, correo y Numero son obligatorios']);
        exit;
    }
    ...
    if ($id !== null) {
        ...
        $stmt = $conn->prepare("INSERT INTO funcionario (ID_Funcionario, Nombre, Apellido, correo, Numero) VALUES (?, ?, ?, ?, ?)");
        
        // Vincula parámetros (i = integer, s = string, s = string, s = string, s = string)
        $stmt->bind_param("issss", $id, $nombre, $apellido, $correo, $numero);
        ...
        $stmt = $conn->prepare("INSERT INTO funcionario (Nombre, Apellido, correo, Numero) VALUES (?, ?, ?, ?)");
        
        // Vincula parámetros (s = string, s = string, s = string, s = string)
        $stmt->bind_param("ssss", $nombre, $apellido, $correo, $numero);
...
if ($method === 'PUT') {
    ...
    if (!$id || (!$nombre && !$apellido && !$correo && !$numero)) {
        http_response_code(400);
