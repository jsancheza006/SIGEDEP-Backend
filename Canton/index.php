<?php
/**
 * API REST para gestionar la tabla 'canton'
 *
 * Descripción:
 *   Endpoint REST de solo lectura para obtener datos de la tabla `canton`.
 *
 * Tabla: canton
 ...
 *   - GET /Canton/index.php                    → Obtiene todos los cantones
 *   - GET /Canton/index.php?id=<ID>            → Obtiene un cantón por ID
 */
...
/**
 * GET: Obtiene cantones
 *
 * Comportamiento:
 *   - Sin parámetros: Retorna un array JSON con todos los cantones
 *   - Con parámetro ?id=<ID>: Retorna el cantón específico o un objeto vacío si no existe
 */
if ($method === 'GET') {
    // Extrae parámetros de la query string (?id=1&nombre=test)
    parse_str($_SERVER['QUERY_STRING'], $query);

    // Si se proporciona un ID, obtiene un cantón específico
    if (isset($query['id'])) {
        $id = (int)$query['id'];

        // Prepara la consulta con un parámetro placeholder (?)
        $stmt = $conn->prepare("SELECT ID_Canton, Nombre, ID_Provincia FROM canton WHERE ID_Canton = ?");
        ...
        // Retorna el cantón o un objeto vacío si no existe
        echo json_encode($result ?: new stdClass());
        exit;
    }

    // Si no hay ID, obtiene todos los cantones
    $result = $conn->query("SELECT ID_Canton, Nombre, ID_Provincia FROM canton");

    // Retorna todos los cantones como un array JSON
