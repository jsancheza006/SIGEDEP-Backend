<?php
/**
 * API REST para gestionar la tabla 'distrito'
 *
 * Descripción:
 *   Endpoint REST de solo lectura para obtener datos de la tabla `distrito`.
 *
 * Tabla: distrito
 ...
 *   - GET /Distrito/index.php                    → Obtiene todos los distritos
 *   - GET /Distrito/index.php?id=<ID>            → Obtiene un distrito por ID
 */
...
/**
 * GET: Obtiene distritos
 *
 * Comportamiento:
 *   - Sin parámetros: Retorna un array JSON con todos los distritos
 *   - Con parámetro ?id=<ID>: Retorna el distrito específico o un objeto vacío si no existe
 */
if ($method === 'GET') {
    // Extrae parámetros de la query string (?id=1&nombre=test)
    parse_str($_SERVER['QUERY_STRING'], $query);

    // Si se proporciona un ID, obtiene un distrito específico
    if (isset($query['id'])) {
        $id = (int)$query['id'];

        // Prepara la consulta con un parámetro placeholder (?)
        $stmt = $conn->prepare("SELECT ID_Distrito, Nombre, ID_Provincia, ID_Canton FROM distrito WHERE ID_Distrito = ?");
        ...
        // Retorna el distrito o un objeto vacío si no existe
        echo json_encode($result ?: new stdClass());
        exit;
    }

    // Si no hay ID, obtiene todos los distritos
    $result = $conn->query("SELECT ID_Distrito, Nombre, ID_Provincia, ID_Canton FROM distrito");

    // Retorna todos los distritos como un array JSON
