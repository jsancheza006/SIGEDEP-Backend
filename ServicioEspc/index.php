 ...
 *   - GET /servicios.php                    → Obtiene todos los servicios
 *   - GET /servicios.php?id=<ID>            → Obtiene un servicio por ID
 *   - POST /servicios.php                   → Crea un nuevo servicio
 *   - PUT /servicios.php?id=<ID>            → Actualiza un servicio existente
 *   - DELETE /servicios.php?id=<ID>         → Elimina un servicio
 * 
 */
...
/**
 * GET: Obtiene servicios
 * 
 * Comportamiento:
 *   - Sin parámetros: Retorna un array JSON con todos los servicios
 *   - Con parámetro ?id=<ID>: Retorna el servicio específico o un objeto vacío si no existe
 * 
 * Respuesta exitosa (GET all):
 *   HTTP 200
 *   [
 *     {"ID_Servicio": 1, "Nombre": "Servicio A"},
 *     {"ID_Servicio": 2, "Nombre": "Servicio B"}
 *   ]
 * 
...
if ($method === 'GET') {
    ...
    if (isset($query['id'])) {
        ...
        $stmt = $conn->prepare("SELECT ID_Servicio, Nombre FROM servicios WHERE ID_Servicio = ?");
    ...
    // Si no hay ID, obtiene todos los servicios
    $result = $conn->query("SELECT ID_Servicio, Nombre FROM servicios");
 ...
 *   {
 *     "ID_Servicio": 5,                    // Opcional: si no se proporciona, se auto-incrementa
...
if ($method === 'POST') {
    ...
    $id = $body['ID_Servicio'] ?? null;          // Opcional
    ...
    if ($id !== null) {
        ...
        $stmt = $conn->prepare("INSERT INTO servicios (ID_Servicio, Nombre) VALUES (?, ?)");
        ...
        if ($stmt->execute()) {
            echo json_encode(['message' => 'Servicio creado correctamente', 'id' => $id]);
        } else {
        ...
        if ($stmt->execute()) {
            ...
            echo json_encode(['message' => 'Servicio creado correctamente', 'id' => $stmt->insert_id]);
        } else {
...
if ($method === 'PUT') {
    ...
    $stmt = $conn->prepare("UPDATE servicios SET Nombre = ? WHERE ID_Servicio = ?");
